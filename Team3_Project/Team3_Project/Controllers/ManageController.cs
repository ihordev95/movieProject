...
namespace Team3_Project.Controllers {
	...
	public class ManageController : Controller {
		...
		public async Task<ActionResult> ManageLogins(ManageMessageId? message) {
			...
			System.Collections.Generic.IList<UserLoginInfo> userLogins = await this.UserManager.GetLoginsAsync(this.User.Identity.GetUserId());
			System.Collections.Generic.List<AuthenticationDescription> otherLogins = this.AuthenticationManager.GetExternalAuthenticationTypes().Where(auth => userLogins.All(ul => auth.AuthenticationType != ul.LoginProvider)).ToList();
