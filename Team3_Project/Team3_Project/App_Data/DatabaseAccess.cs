...
public class DatabaseAccess {
	...
	private readonly String conn = "server=memdixyp.mysql.db.hostpoint.ch;uid=memdixyp_user;pwd=rmpvzSp4BsNm;database=memdixyp_imdb;SslMode=None"; //read only
	private readonly String conn2 = "server=memdixyp.mysql.db.hostpoint.ch;uid=memdixyp_user;pwd=rmpvzSp4BsNm;database=memdixyp_film;SslMode=None"; //writable
	...
	public DataSet InsertDataRow(String s , Int32 i)   //Example function that would insert a row into the table with values of s and i
	{
		try {
			MySqlConnection connection = new MySqlConnection(this.conn);     //Make a new connection using our connection string
			DataSet ds = new DataSet();
			...
			MySqlCommand cmd = new MySqlCommand("<stored procedure name>" , connection) {
				CommandType = CommandType.StoredProcedure  //This tells it we are using a stored procedure instead of passing in a query of our own
			};
